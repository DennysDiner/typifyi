...
def auth(creds: HTTPBasicCredentials | None = Depends(security)) -> None:
    ...
    if not st.web_auth_user:
        return
    ok = creds is not None and secrets.compare_digest(creds.username, st.web_auth_user) and secrets.compare_digest(creds.password, st.web_auth_password or "")
    if not ok:
...
def feed(request: Request, authority: str = "", type_: str = "", portfolio: str = "", since: str = "", exemption: str = "", q: str = "", page: int = 1):
    ...
    return render(request, "feed.html", rows=rows, auths=auths, page=page, filters=dict(authority=authority, type_=type_, portfolio=portfolio, since=since, exemption=exemption, q=q))
...
def item(request: Request, item_id: int):
    ...
    if not it:
        raise HTTPException(404)
    docs = c.execute("SELECT d.*, c.retrieved_at, c.http_status, c.final_url, c.headers, b.path FROM documents d JOIN captures c ON c.id=d.capture_id JOIN blobs b ON b.sha256=d.sha256 WHERE d.item_id=? ORDER BY d.id", (item_id,)).fetchall()
...
@app.post("/item/{item_id}/tag", dependencies=[Depends(auth)])
...
def search(request: Request, q: str = ""):
    ...
    if q:
        rows = c.execute(
            "SELECT d.id AS document_id, d.item_id, d.url, d.sha256, i.title, a.name AS authority_name, snippet(document_fts, 0, '<mark>', '</mark>', ' … ', 24) AS snip"
...
def applications(request: Request):
    ...
    for a in c.execute("SELECT * FROM applications ORDER BY lodged_date DESC"):
        ...
        apps.append(dict(a=a, st=st, next=nxt, days=days, links=links))
